...
class ModLS():
    ...
    def __init__(self, ficLisUni=None, ficMod=None, red=None,
                 funcLoss=nll_loss, Optim=lambda params: SGD(params, lr=1.e-2), device=None, ficLog=None):
        if not device:
    ...
    def addEval(self, señal):
        ...
        self.loss += self.funcLoss(salida, trn, reduction='sum').item()
        self.numUni += len(trn)
    ...
    def printEval(self, epo):
        ...
        print(f'{epo=:{".1f" if isinstance(epo, float) else "d"}}\t{self.lr=:.1f}\t{self.loss=:.4f}\t{self.corr=:.2%}\t({dt.now():%d/%b/%y %H:%M:%S})\n', flush=True, end='')
    ...
    def entrena(self, lotesEnt, lotesDev=None, epo=0, numEval=10):
        ...
        for loteAct, lotes in enumerate(tqdm.tqdm(lotesEnt, ascii=' >=')):
            lossTot = numUni = numSen = corr = lr = 0
            lossAct = 0
            totSen = 0

            def closure():
                nonlocal lossTot, numUni, numSen, corr, lr
                nonlocal lossAct, totSen
                ...
                for señal in lote:
                    ...
                    prm = señal.prm.to(self.device)
                    salida = self.red(prm).squeeze()
                    #loss = self.funcLoss(salida, trn, reduction='sum')
            ...
            lr = self.optim.step(closure=closure)
            self.lr = -1

            if self.fpLog:
                lossTot = float(lossTot) / totSen
                corr = float(corr) / numUni
                print(f'{lossTot=:.4f}\t{corr=:.2%}', file=self.fpLog)

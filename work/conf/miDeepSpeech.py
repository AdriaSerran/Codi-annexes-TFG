...
class MiDeepSpeech(torch.nn.Module):
    ...
    *Deep Speech: Scaling up end-to-end speech recognition* :cite:`hannun2014deep`.
    ...
    def __init__(
        ...
        super(MiDeepSpeech, self).__init__()
        ...
        self.bi_rnn = torch.nn.RNN(n_hidden, n_hidden, num_layers=1, nonlinearity="tanh", bidirectional=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #x = x - x.min() + 1.e-6 / x.abs().max() + 1e-20
        # N x T x n_class
        x = torch.nn.functional.log_softmax(x, dim=2)
        #if torch.any(torch.isnan(x)) or torch.any(torch.isinf(x)):
            ##print(f'{torch.any(torch.isnan(x0))=}\t{torch.any(torch.isinf(x0))=}')
            #print(f'{torch.any(torch.isnan(x1))=}\t{torch.any(torch.isinf(x1))=}')
            #print(f'{torch.any(torch.isnan(x2))=}\t{torch.any(torch.isinf(x2))=}')
            #print(f'{torch.any(torch.isnan(x3))=}\t{torch.any(torch.isinf(x3))=}')
            #print(f'{torch.any(torch.isnan(x4))=}\t{torch.any(torch.isinf(x4))=}')
